 ...
 * by their names.
 * <p>
 * This class embeds conventions for naming PDR "archive preservation package"
 * (AIP) files--i.e., preservation bags. There are two conventions supported:
 * <ul>
 * <li>Original form: <i>identifier</i>.mbag<i>M</i>_<i>N</i>-<i>S</i> <br>
 * where <i>M</i>_<i>N</i> is the Multibag profile (MbP) version (e.g.
 * <tt>0_2</tt>, <tt>0_4</tt>), <br>
 * and <i>S</i> is the sequence number (e.g. 0, 1, ...).</li>
 * <li>As of MbP 0.4: <i>identifier</i>.<i>V</V>_<i>V</V>_<i>V</V>.mbag<i>M</i>_
 * <i>N</i>-<i>S</i> <br>
 * where <i>V</V>_<i>V</V>_<i>V</V> is the dataset release version (e.g.,
 * <tt>1_0_0</tt>, <tt>2_1_10</tt>)
 * </ul>
 * These conventions are used to determine which bags are associated with a
 * given identifier (and version) and which are the head bags.
 */
public class BagUtils {
    ...
    /**
     * parse a bag name into its meaningful components: id, version, multibag
     * profile version, multibag sequence number, and serialization extension
     * (if present). The version field will be an empty string.
     * 
     * @param name
     *            the name to parse. This name should not include any preceeding
     *            file paths.
     * @return List<String> - a list containing the components in order of id,
     *         version, multibag profile version, multibag sequence number, and
     *         serialization extension. If the name does not contain a
     *         serialization extension, the fourth element will be an empty
     *         string. That field will not include a leading dot. The version
     *         field will be an empty string.
     * @throws ParseException
     *             if the given name does not match the accepted pattern for bag
     *             names
     */
    ...
    /**
     * parse a bag name (according the format that includes the AIP version)
     * into its meaningful components: id, version, multibag profile version,
     ...
     *            file paths.
     * @return List<String> - a list containing the components in order of id,
     *         multibag profile version, multibag sequence number, and
     *         serialization extension. If the name does not contain a
     *         serialization extension, the fourth element will be an empty
     *         string. That field will not include a leading dot.
     * @throws ParseException
     *             if the given name does not match the accepted pattern for bag
     *             names
     */
    ...
    /**
     * parse a bag name (according the format that excludes the AIP version)
     * into its meaningful components: id, version, multibag profile version,
     * multibag sequence number, and serialization extension (if present). The
     * version fields will be exactly as they appear in the name (i.e. with
     * underscore, _, as the delimiter).
     * 
     * @param name
     *            the name to parse. This name should not include any preceeding
     *            file paths.
     * @return List<String> - a list containing the components in order of id,
     *         version, multibag profile version, multibag sequence number, and
     *         serialization extension. If the name does not contain a version
     *         or serialization extension, the second or fourth element,
     *         respectively, will be an empty string. The extension field will
    ...
    /**
     * return true if the file is a legal bag name. The name may or may not
     * contain a serialization extension (e.g. ".zip").
     * 
     * @param name
     *            the bag name to judge
     */
    ...
    /**
     * return the version of the multibag profile a preservation bag claims to
     * conform to based on its name.
     * 
     * @param name
     *            the bag's name
     * @return String - the dot-delimited version string, or an empty string if
     *         the version cannot be determined.
     */
    public static String multibagVersionOf(String name) {
	try {
	    return usdelim.matcher(parseBagName(name).get(2)).replaceAll(".");
    ...
    /**
     * return a Comparator for sorting version strings. In this implementation,
     * a version is a String containing a sequence of integers delimited either
     * by periods ('.') or underscores ('_'). If the fields are anything else
     * but integers, a NumberFormatException is thrown.
     */
    ...
    /**
     * return the latest head bag from the List of bag names. Each item in the
     * list must be a legal bag name (see isLegalBagName()) and have the same
     * identifier. If the former requirement is not satisfied, a runtime
     * exception will be thrown; if the second is not, the result is undefined.
     * 
     * @param bagnames
     *            the list of bag names.
     * @return String - the name of the latest head bag in the list
     * @throws IllegalArgumentException
     *             if the list is empty or contains an illegal bag name
     */
    ...
    /**
     * select the bags from a list of bagnames that match a desired version.
     * Under certain circumstances, this will look for certain varients. If none
     * of the names match the version, an empty list is returned.
     * 
     * @param bagnames
     *            the list of bag names to filter
     * @param version
     *            the desired version in dot-delimited form
     * @return List<String> - the list of matching names (in the same order as
	...
	// Most likely given current NIST practice, if version is simply "0" or
	// "1",
	...
	while (true) {
	    ...
	    vernamere = (version.length() == 0) ? Pattern.compile("^(\\w+)\\.mbag")
