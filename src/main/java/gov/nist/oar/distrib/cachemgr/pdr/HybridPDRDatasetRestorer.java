...
public class HybridPDRDatasetRestorer implements Restorer, PDRConstants, PDRCacheRoles {
    ...
    private long smszlim = 100000000L; // Default size limit for small files (100 MB)
    private Logger log;

    /**
     * Create the restorer with dual storage and head bag cache using default settings.
     *
     * @param publicLtstore     The public long-term storage where some head bags are stored.
     * @param restrictedLtstore The restricted long-term storage where some head bags are stored.
     * @param hbcm              The cache manager to use for caching the head bags.
     */
    public HybridPDRDatasetRestorer(BagStorage publicLtstore, BagStorage restrictedLtstore, HeadBagCacheManager hbcm) {
        this(publicLtstore, restrictedLtstore, hbcm, -1L, null);
    }

    /**
     * Create the restorer with dual storage, head bag cache, and a specified small size limit.
     *
     * @param publicLtstore     The public long-term storage where some head bags are stored.
     * @param restrictedLtstore The restricted long-term storage where some head bags are stored.
     * @param hbcm              The cache manager to use for caching the head bags.
    ...
    public HybridPDRDatasetRestorer(BagStorage publicLtstore, BagStorage restrictedLtstore, HeadBagCacheManager hbcm,
                                    long smallsizelim) {
        this(publicLtstore, restrictedLtstore, hbcm, smallsizelim, null);
    }

    /**
     * Create the restorer with dual storage, head bag cache, a specified small size limit, and a logger.
     *
     * @param publicLtstore     The public long-term storage where some head bags are stored.
     * @param restrictedLtstore The restricted long-term storage where some head bags are stored.
     * @param hbcm              The cache manager to use for caching the head bags.
    ...
    public HybridPDRDatasetRestorer(BagStorage publicLtstore, BagStorage restrictedLtstore, HeadBagCacheManager hbcm,
        ...
        this.publicLtstore = publicLtstore;
        this.restrictedLtstore = restrictedLtstore;
        this.hbcm = hbcm;
        this.smszlim = smallsizelim > 0 ? smallsizelim : this.smszlim;
        this.log = logger != null ? logger : LoggerFactory.getLogger(getClass().getName());
    }
    ...
    public boolean doesNotExist(String id) throws StorageVolumeException, CacheManagementException {
        // Check in restricted storage first
        if (!doesNotExistInStorage(id, restrictedLtstore)) {
            return false;  // Object exists in restricted storage
        }
        // Check in public storage
        return doesNotExistInStorage(id, publicLtstore);
    }

    private boolean doesNotExistInStorage(String id, BagStorage storage) throws StorageVolumeException,
            CacheManagementException {
        String[] parts = parseId(id);
        try {
            storage.findBagsFor(parts[0]);
            return false;
    ...
    public void restoreObject(String id, Reservation resv, String name, JSONObject metadata)
        ...
        boolean isRestored = tryRestoreFromStorage(id, idparts, restrictedLtstore, resv, name, metadata);
        if (!isRestored) {
            isRestored = tryRestoreFromStorage(id, idparts, publicLtstore, resv, name, metadata);
        }

        if (!isRestored) {
            throw new ObjectNotFoundException("Resource " + id + " not found in any storage");
        }
    }

    /**
     * Attempts to restore an object from a specified storage.
     */
    private boolean tryRestoreFromStorage(String id, String[] idparts, BagStorage storage, Reservation resv,
                                          String name, JSONObject metadata)
            throws RestorationException, StorageVolumeException, JSONException {
        try {
            String headbag = storage.findHeadBagFor(idparts[0], idparts[2]);
            JSONObject cachemd = getCacheMDFromHeadBag(headbag, idparts[1]);
            if (cachemd == null) {
                return false;
            }
            if (metadata != null) {
            ...
            try (InputStream bstrm = storage.openFile(srcbag)) {
                ZipBagUtils.OpenEntry ntry = ZipBagUtils.openDataFile(bstrm, bagname, idparts[1]);
                resv.saveAs(ntry.stream, id, name, cachemd);
                log.info("Cached " + id);
                return true;
            } catch (FileNotFoundException ex) {
                // Log and continue, might be found in the other storage
                log.warn("File not found in " + storage.toString() + ": " + ex.getMessage());
            } catch (IOException ex) {
                throw new RestorationException(id + ": Trouble reading data file: " + ex.getMessage(), ex);
            }
        } catch (ResourceNotFoundException | CacheManagementException ex) {
            // Log and continue, might be found in the other storage
            log.warn("Issue with storage " + storage.toString() + ": " + ex.getMessage());
        }
        return false;
    }
    ...
    public Set<String> cacheDataset(String aipid, String version, Cache into, boolean recache, int prefs, String target)
        ...
        Set<String> cachedFiles = tryCacheDatasetFromStorage(aipid, version, into, recache, prefs, target,
                restrictedLtstore);
        if (cachedFiles.isEmpty()) {
            // If the dataset is not fully cached from restricted storage, try public storage
            cachedFiles = tryCacheDatasetFromStorage(aipid, version, into, recache, prefs, target, publicLtstore);
        }

        return cachedFiles;
    }

    private Set<String> tryCacheDatasetFromStorage(String aipid, String version, Cache into, boolean recache,
                                                   int prefs, String target, BagStorage storage)
            throws StorageVolumeException, ResourceNotFoundException, CacheManagementException {

        // find the head bag in the bag store
        String headbag = storage.findHeadBagFor(aipid, version);  // throws exc if does not exist
    ...
    protected void cacheFromBag(String bagfile, Collection<String> need, Collection<String> cached,
        ...
        if (!tryCacheFromBag(bagfile, need, cached, resmd, defprefs, forVersion, into, recache, target,
                restrictedLtstore)) {
            // If caching from restricted storage was not fully successful, try public storage
            tryCacheFromBag(bagfile, need, cached, resmd, defprefs, forVersion, into, recache, target, publicLtstore);
        }
    }

    private boolean tryCacheFromBag(String bagfile, Collection<String> need, Collection<String> cached,
                                    JSONObject resmd, int defprefs, String forVersion, Cache into,
                                    boolean recache, String target, BagStorage storage)
            throws StorageVolumeException, FileNotFoundException, CacheManagementException {

        boolean allFilesCached = false; // flag to track if all files are cached successfully
