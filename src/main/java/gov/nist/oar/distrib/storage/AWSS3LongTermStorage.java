...
public class AWSS3LongTermStorage extends LongTermStorageBase {
     ...
     *             if there is a problem accessing the S3 service. While this is
     *             a runtime exception that does not have to be caught by the
     *             caller, catching it is recommended to address connection
     *             problems early.
     */
    public AWSS3LongTermStorage(String bucketname, AmazonS3 s3) throws FileNotFoundException, AmazonServiceException {
	bucket = bucketname;
	s3client = s3;
	...
	if (s3Object == null) {
	    ...
	    if (getSize(filename) > 5000000) // 10x smaller limit than for local
					     // files
		...
		throw new StorageStateException(
			"Unable to calculate checksum for small file: " + filename + ": " + ex.getMessage());
	    }
	}
	...
	try (InputStreamReader csrdr = new InputStreamReader(s3Object.getObjectContent())) {
	    ...
	    throw new StorageStateException(
		    "Failed to read cached checksum value from " + filename + ".sha256" + ": " + ex.getMessage(), ex);
	}
    }
     ...
     */
    @Override
    public List<String> findBagsFor(String identifier) throws ResourceNotFoundException, StorageStateException {
