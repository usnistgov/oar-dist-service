...
/**
 * a file description with extra support for properties describing OAR
 * preservation bags.
 * <p>
 * In the OAR PDR model, an <em>archive information package</em> (AIP) is made
 * up of one or more files. Together, these make up the AIP. AIP files,
 * therefore, have associated metadata that indicate their membership in a
 * particular AIP (e.g. AIP identifier, version, etc.) and which are are
 * important for accessing them in storage systems. The OAR-PDR (at NIST)
 * follows a naming convention that embeds some of this metadata (which is
 * encasulated in the {@link gov.nist.oar.bags.preservation.BagUtils BagUtils}
 * class). This specialized {@link gov.nist.oar.distrib.FileDescription
 * FileDescription} will add this embedded information as additional properties.
 */
public class BagDescription extends FileDescription {
    ...
    /**
     * add additional properties to an FileDescription that have been extracted
     * from a a preservation bag name that follows the OAR bag naming
     * conventions. If the name is not a compliant name, this method exits
     * without adding anything.
     * 
     * @param desc
     *            the description to add the properties to
     * @param bagname
     *            a preservation bag filename to parse for information. This can
