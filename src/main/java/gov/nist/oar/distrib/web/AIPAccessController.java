...
/**
 * A web service controller that provides access to Archive Information Package
 * (AIP) files.
 * <p>
 * In the OAR PDR model, an <em>archive information package</em> (AIP) is made
 * up of one or more files. Together, these make up the AIP. AIP files have
 * unique names within the service; if one knows the name, one can access it. In
 * this service implementation, AIP files are in the form of serialized
 * <em>preservation bags</em>--files representing BagIt-formatted data packages.
 * <p>
 * This controller allows access to an AIP file based on its name, allowing
 * download of the file itself or metadata about it (encoded in JSON). It
 * provides a web front end to bag file access provided by the
 * {@link gov.nist.oar.distrib.service.PreservationBagService
 * PreservationBagService}. (Note that discovery of bag file names and head
 * bags, courtesy of the
 * {@link gov.nist.oar.distrib.service.PreservationBagService
 * PreservationBagService}, is provided via the DatasetAccessController.)
 *
...
public class AIPAccessController {
     ...
     * 
     * @param name
     *            the name of the desired AIP file
     * @param resp
     *            the response object. This will be used to set the response
     *            header and stream the desired file.
     */
    @ApiOperation(value = "Return an AIP file (e.g. a preservation bag)", nickname = "getAIPFile", notes = "return the AIP with the given name")
    @GetMapping(value = "/{name}")
    public void downloadAIP(@PathVariable("name") String name, @ApiIgnore HttpServletResponse resp)
	...
	try {
	    ...
	    resp.setHeader("Content-Disposition",
		    "filename=\"" + Pattern.compile("/+").matcher(name).replaceAll("_") + "\"");
		...
		if (ex.getMessage().contains("Connection reset by peer")) {
		    ...
		    logger.error("IO error while sending file, " + name + ": " + ex.getMessage());
		    throw ex;
		}
	    }
     ...
     *            the name of the desired AIP file
     */
    @ApiOperation(value = "describe an AIP file (e.g. a preservation bag)", nickname = "describeAIPFile", notes = "return a description of the AIP with the given name as a JSON object")
	...
	try {
		...
		try {
		    // set a download URL for the file being described as a
		    // property
