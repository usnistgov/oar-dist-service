 ...
 * <p>
 * This interface is provides a common interface for accessing storage.  Not only may the detailed API to the underlying
 * storage may differ (e.g. AWS S3 versus local file systems) but the conventions for how data is organized on the systems 
 * may differ.  This interface hides those difference.  
 * <p>
 * Files are stored in a storage system with a filename that is unique only to that instance of the storage system; a file
 * and its metadata can be access via that file name.  Generally any kind of file can be stored in the system, but a the 
 * main purpose of the storage system is to store "archive information packages" (AIPs); each one is identified with a 
 * repository-unique AIP identifier.  Each AIP is made up of one or more files, stored as preservation bags (using the 
 * BagIt standard).  To retrieve a bag, one must learn what it's file name is; methods like 
 * {@link #findBagsFor(String) findBagsFor()} and {@link #findHeadBagFor(String, String) findHeadBagFor()} can map an AIP ID
 * to bag names.  In particular, each version of an AIP has, among its bags, a designated "head bag", which contains 
 * important AIP metadata and information about the other bags that are part of the AIP.  The 
 * {@link #findHeadBagFor(String, String) findHeadBagFor()} will return the filename for the serialized head bag for the 
 * AIP with a given identifier and version.  To determine the name of other (non-bag) types of files, one must have an 
