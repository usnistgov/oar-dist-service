...
public class ObjectUtilsTest {
	...
	assertFalse("Pay attention to field boundaries",
		ObjectUtils.isAllowedURL("https://s3.amazonaws.com/nist-midas-games/doom.zip", allowed));
    }
	...
	UrlStatusLocation urlLoc = ObjectUtils.getURLStatus(connect);
	assertEquals(urlLoc.getStatus(), 404);

	obj = new URL(testUrlRedirect);
	connect = (HttpURLConnection) obj.openConnection();

	urlLoc = ObjectUtils.getURLStatus(connect);
