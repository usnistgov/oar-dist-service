...
public class FromBagFileDownloadServiceTest {
	...
	for (String file : zips) {
		...
		try (FileOutputStream os = new FileOutputStream(new File(testdir.toFile(), file))) {
    ...
    @Test(expected = ResourceNotFoundException.class)
    public void testGetDataFileBadID() throws ResourceNotFoundException, DistributionException, FileNotFoundException {
